"""Takes random part of the original video and overlays target audio on it/"""
import math
import os
import argparse
...
def random2_baseline(
    ...
    while len(new_video) < max_length:
        id = id % len(all_segment_ids)
        new_video.extend(video[id * S + (W - S) : id * S + W])
        id += 1
