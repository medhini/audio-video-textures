    ...
    def forward(self, frame0, frame1, TP):
        ...
        with torch.no_grad():
            ...
            for intermediateIndex in range(1, self.SF):
                ...
                intrpOut = self.ArbTimeFlowIntrp(torch.cat((I0, I1, F_0_1, F_1_0, \
                            F_t_1, F_t_0, g_I1_F_t_1, g_I0_F_t_0), dim=1))
                ...
                for batchIndex in range(BATCH_SIZE):
                    output_frames.append(TP(Ft_p[batchIndex].cpu().detach()).resize(self.origDim, Image.BILINEAR))
                frameCounter += 1
